...
“”“Search Kalshi markets. api.elections.kalshi.com serves ALL markets including sports.”””
try:
...
ticker: market ticker e.g. FED-25JUN-T5.25
side: yes or no
...
return {“error”: f”Order exceeds $5.00 max bet limit. Cost: ${cost_dollars:.2f}. Reduce count or price.”}
...
return {
“open_orders”: [{“order_id”: o.get(“order_id”), “ticker”: o.get(“ticker”),
“side”: o.get(“side”), “action”: o.get(“action”),
“count”: o.get(“count”), “yes_price”: f”${o.get(‘yes_price’, 0)/100:.2f}”,
...
# Fetch all sport categories in parallel from the single Kalshi endpoint
...
DB_PATH = os.getenv(“DB_PATH”, os.path.join(os.path.dirname(**file**), “finsight_trades.db”))
...
1. If session quality is “poor” or “none” — warn the user strongly and stop. Do not proceed.
1. If expected move is >70% used — flag as late entry, reduce confidence by 2 points minimum.
1. THEN call get_options_chain — get strikes with Greeks. Target delta 0.30–0.50 only for premium trading. Reject strikes outside this range.
...
## PREDICTION MARKETS & KALSHI TRADING

You are also an expert prediction market analyst with the ability to place REAL bets on Kalshi. Real money is at stake — be disciplined.
...
- Always check get_kalshi_balance before placing any order
- Always show the full prediction market summary table before executing
- After placing an order, call get_kalshi_orders to confirm it’s resting

When asked about prediction markets, events, or probabilities:
...
1. Build your own probability estimate using Bayesian reasoning (calculate_bayesian_probability) — start with a historical base rate, then update with current evidence. Show your reasoning chain.
1. Compare your estimate to the market’s implied probability. The difference is your EDGE.
1. If edge > 5%, calculate optimal bet size with calculate_kelly_criterion.
1. Always present a prediction market recommendation in this format:
...
“description”: “Place a REAL Kalshi prediction market order. Max $5 per bet. Always show the full prediction market summary table and get approval before calling this unless confidence >= 7 AND cost <= $2.”,
“input_schema”: {
“type”: “object”,
“properties”: {
“ticker”:          {“type”: “string”, “description”: “Kalshi market ticker e.g. FED-25JUN-T5.25”},
“side”:            {“type”: “string”, “enum”: [“yes”, “no”], “description”: “Which side to bet”},
“count”:           {“type”: “integer”, “description”: “Number of contracts (1 contract = yes_price_cents / 100 dollars)”},
“yes_price_cents”: {“type”: “integer”, “description”: “Price in cents (1-99). E.g. 55 = $0.55 per contract”},
