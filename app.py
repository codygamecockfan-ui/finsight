...
params={"adjusted": "true", "sort": "asc", "limit": 500, "apiKey": POLYGON_API_KE
timeout=10
)
...
price = snap.get("latestTrade", {}).get("p") or snap.get("dailyBar", {}).get("c") o
...
pct_used = round((already_moved / expected_move) * 100, 1) if expected_move
return {
"ticker": ticker,
"current_price": price,
...
"signal": "late — most of the move is done" if pct_used > 70 else "ro
...
def init_db():
os.makedirs(os.path.dirname(DB_PATH), exist_ok=True) if os.path.dirname(DB_PATH) else Non
...
EVERY single response that involves a trade recommendation, trade execution, or market analys
You are FinSight, an elite AI trading assistant with deep expertise in global markets,
geopolitical analysis, macroeconomics, and active trading — specializing in 0DTE and short-da
...
You have a dry wit and a foul mouth. You swear naturally and casually (shit, damn, hell, ass,
the way a real trader would on a desk — not gratuitously, but it comes out when the market do
something stupid, when a trade goes wrong, or when you're making a point. Never forced, alway
You are NOT folksy, country, or corny. Think Bloomberg terminal meets a guy who's been in the
## RESPONSE STYLE
- Keep responses CONCISE. Get to the point fast.
- No long-winded intros or summaries. Lead with the insight or the data.
- For ANY trade recommendation or market analysis, ALWAYS include a summary table in this for
| Parameter | Value |
|-----------|-------|
| Ticker | |
| Asset Type | |
...
THE TABLE COMES FIRST. Show the full table before executing, before confirming, before On auto-executes: show table first, then execute, then confirm it was fired.
...
- Always end trade recommendations with: *Paper trading only — not financial advice.*
anythi
## DATA & SOURCING RULES
...
- Do NOT call get_trading_session, get_vwap, or get_expected_move — these are equity-only too
...
1. get_trading_session — check if timing is favorable. If session quality is "poor", warn the
2. get_vwap (for the ticker) — note whether price is above or below VWAP and by how much.
3. get_expected_move (for the ticker) — note how much of the daily range is already used. If
4. get_options_chain — get the chain with Greeks. Target delta 0.30–0.50 only. Reject strikes
Include session, VWAP position, expected move status, and delta in the summary table for all
...
- Confidence >= 7 AND dollar amount > $200: present the trade summary and ask "Want me to fir
- Confidence < 7: NEVER auto-execute. Always present summary and ask for approval. State your
...
- NEVER recommend or place an options contract with an ask price above $0.80/contract ($80 to
- The options chain already filters these out. If no affordable contracts exist, tell the use
...
- Delta MUST be between 0.30 and 0.50 for 0DTE entries. Outside this range = reject or - These are hardcoded rules. Apply defaults automatically and tell the user if overriding the
flag s
...
- When executing, use place_paper_trade then immediately call set_trade_monitor with exit rul
- Default monitor rules if none specified: stop_loss_pct=0.15, take_profit_pct=0.25, time_lim
...
- Always explain the market mechanism — not just "oil is up", but WHY and what it means to tr
## WHAT YOU DO NOT DO
...
"description": "Get the REAL-TIME stock price, bid/ask, and latest trade info for a t
},
{
},
{
...
"type": "object",
"properties": {
"ticker": {"type": "string", "description": "Stock or crypto ticker e.g. AAPL
},
"required": ["ticker"]
}
"name": "get_options_chain",
"description": "Get the options chain for a ticker including available strike prices
"input_schema": {
"type": "object",
"properties": {
"ticker": {"type": "string", "description": "Stock ticker symbol"},
"expiration_date": {"type": "string", "description": "Options expiration date
...
"description": "Place a PAPER trade via Alpaca. Only call after explicit user confirm
"input_schema": {
"type": "object",
"properties": {
"symbol": {"type": "string", "description": "Ticker or crypto symbol
"side": {"type": "string", "enum": ["buy","sell"]},
"dollar_amount": {"type": "number", "description": "Dollar amount to invest"
"asset_type": {"type": "string", "enum": ["stock","option","crypto"]},
"current_price": {"type": "number", "description": "Current price for qty ca
"thesis": {"type": "string", "description": "Brief trade thesis — why
"confidence": {"type": "integer", "description": "Confidence level 1-10"},
"indicators": {"type": "string", "description": "Key indicators that trig
...
"description": "Get paper trading account balance, buying power, and portfolio "input_schema": {"type": "object", "properties": {}, "required": []}
value.
...
"description": "Set autonomous exit rules for a position. Call immediately after plac
"input_schema": {
},
{
},
...
"stop_loss_pct": {"type": "number", "description": "e.g. 0.10 = 10% loss. U
"take_profit_pct": {"type": "number", "description": "e.g. 0.20 = 20% gain. U
"time_limit_min": {"type": "integer", "description": "Minutes until forced s
},
"required": ["symbol","entry_price","asset_type","stop_loss_pct","take_profit_pct
}
"name": "get_monitor_log",
"description": "Get the log of autonomous trade actions taken by the background monit
"input_schema": {"type": "object", "properties": {}, "required": []}
...
"description": "Get aggregate performance stats from the trade journal — win rate, to
"input_schema": {"type": "object", "properties": {}, "required": []}
"name": "get_recent_trades",
"description": "Get the last N trades from the journal with full details including th
"input_schema": {
"type": "object",
"properties": {
"limit": {"type": "integer", "description": "Number of recent trades to retur
},
"required": []
}
"name": "get_trading_session",
"description": "Get the current trading session window and quality rating for 0DTE tr
"input_schema": {"type": "object", "properties": {}, "required": []}
{
— tell
"name": "get_vwap",
"description": "Calculate intraday VWAP for a stock ticker. Essential for 0DTE "input_schema": {
"type": "object",
"properties": {
...
"description": "Calculate the market-implied expected move for today using ATM "input_schema": {
"type": "object",
"properties": {
"ticker": {"type": "string", "description": "Stock ticker e.g. SPY, QQQ"
"option_type": {"type": "string", "enum": ["call", "put"], "description": "Wh
},
"required": ["ticker", "option_type"]
}
}
...
"prev_close": prev_c,
"change_pct": round(((latest - prev_c) / prev_c) * 100, 2) if latest and prev_c e
"volume": daily.get("v"),
...
try:
trade_r = requests.get(f"{ALPACA_DATA_URL}/stocks/{ticker}/trades/latest", headers=AL
quote_r = requests.get(f"{ALPACA_DATA_URL}/stocks/{ticker}/quotes/latest", headers=AL
snap_r = requests.get(f"{ALPACA_DATA_URL}/stocks/{ticker}/snapshot", headers=AL
...
exp_r = requests.get(
"https://api.tradier.com/v1/markets/options/expirations",
headers=headers, params={"symbol": ticker, "includeAllRoots": "true"}, expirations = exp_r.json().get("expirations", {}).get("date", [])
if not expirations:
...
filtered = [o for o in options if o.get("option_type","").lower() == option_type.lowe
...
"delta": o.get("greeks", {}).get("delta") if o.get("greeks") "gamma": o.get("greeks", {}).get("gamma") if o.get("greeks") "theta": o.get("greeks", {}).get("theta") if o.get("greeks") "iv": o.get("greeks", {}).get("mid_iv") if o.get("greeks") else
else N
else N
else N
})
# Filter out contracts exceeding max premium
affordable = [c for c in contracts if c.get("ask") and c["ask"] <= MAX_OPTIONS_PREMIU
...
"source": "Tradier",
"note": f"Only showing contracts with ask <= ${MAX_OPTIONS_PREMIUM:
}
except Exception as e:
return {"error": str(e)}
...
"published": a["publishedAt"], "summary": a.get("description",""),
for a in articles],
"source": "NewsAPI.org"
}
except Exception as e:
...
try:
snap = requests.get(f"{ALPACA_DATA_URL}/stocks/{ticker}/snapshot", headers=ALPAC
daily = snap.get("dailyBar", {})
prev = snap.get("prevDailyBar", {})
trade = snap.get("latestTrade", {})
...
pc = prev.get("c")
results[ticker] = {
"price": lp, "open": daily.get("o"), "high": daily.get("h"), "low": daily.get
"prev_close": pc,
...
try:
vix = requests.get(f"https://api.polygon.io/v2/aggs/ticker/VIX/prev?adjusted=true&api
if vix.get("resultsCount", 0) > 0:
v = vix["results"][0]
results["VIX"] = {"close": v["c"], "open": v["o"], "note": "Prior day close via P
except:
results["VIX"] = {"error": "Failed to fetch VIX"}
return {"indices": results, "as_of": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "sourc
def get_stock_technicals(ticker: str, days: int = 30) -> dict:
ticker = ticker.upper()
end_date = datetime.now().strftime("%Y-%m-%d")
start_date = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d")
try:
r = requests.get(f"https://api.polygon.io/v2/aggs/ticker/{ticker}/range/1/day/{
...
order_data = {"symbol": f"{symbol}/USD", "notional": str(round(dollar_amount,
...
if qty < 1:
return {"error": f"${dollar_amount} too small to buy 1 share at ${current_pri
order_data = {"symbol": symbol, "qty": str(qty), "side": side, "type": "marke
...
if qty < 1:
return {"error": f"${dollar_amount} too small for 1 contract at ${current_pri
order_data estimated_cost = qty * cpc
qty_display = str(qty)
= {"symbol": symbol, "qty": str(qty), "side": side, "type": "marke
r = requests.post(f"{ALPACA_TRADING_URL}/orders", headers=ALPACA_HEADERS, result = r.json()
json=o
if "id" in result:
# Log to journal
db_log_entry(
...
"note": " PAPER trade — no real money. Logged to trade journal.",
...
try:
r = requests.get(f"{ALPACA_TRADING_URL}/positions", headers=ALPACA_HEADERS, timeout=1
positions = r.json()
if not positions:
...
try:
acct = requests.get(f"{ALPACA_TRADING_URL}/account", headers=ALPACA_HEADERS, t
...
try:
r = requests.delete(f"{ALPACA_TRADING_URL}/positions/{api_symbol}", headers=ALPA
...
try:
pos_r = requests.get(f"{ALPACA_TRADING_URL}/positions/{api_symbol}", headers=
pos = pos_r.json()
...
stop_loss_pct: float, take_profit_pct: float, time_limit_min: int) -> d
...
return None
def auto_close_position(symbol: str, asset_type: str, reason: str, exit_price: float = symbol = symbol.upper().replace("/USD","").replace("USD","")
api_symbol = f"{symbol}USD" if asset_type == "crypto" else symbol
try:
0):
r = requests.delete(f"{ALPACA_TRADING_URL}/positions/{api_symbol}", headers=ALPACA_HE
...
msg = f"[{datetime.now().strftime('%H:%M:%S')}] AUTO-SELL {symbol} | {reason} | {'
except Exception as e:
...
print("[FinSight Monitor] while True:
time.sleep(60)
with monitor_lock:
Autonomous trade monitor started")
...
auto_close_position(symbol, asset_type, f"Take profit (+{pct*100:.2f}%)",
with monitor_lock: trade_monitors.pop(symbol, None)
continue
msg = f"[{now.strftime('%H:%M:%S')}] {symbol} | ${cp:.4f} | {pct*100:.2f}% |
...
return "\n".join(b.text for b in response.content if hasattr(b, "text")) or "Unex
# ─────────────────────────────────────────────
# AUTH
# ─────────────────────────────────────────────
...
"elapsed_min": round((datetime.now()-v["entry_time"]).total_seconds()/60,
